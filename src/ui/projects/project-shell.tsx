    ...
    >
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        ...
        <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Tasks", value: tasks.length, sub: tasks.length > 0 ? `${taskCompletionPct}% done` : "none yet" },
            { label: "Done", value: tasksDone.length, sub: tasksCancelled.length > 0 ? `${tasksCancelled.length} cancelled` : "all clear" },
            { label: "Events", value: events.length, sub: `${upcomingEvents.length} upcoming` },
            { label: "Total time", value: fmtDuration(totalEventMinutes), sub: events.length === 0 ? "no events" : `across ${events.length} events` },
          ].map(({ label, value, sub }) => (
            <div
              key={label}
